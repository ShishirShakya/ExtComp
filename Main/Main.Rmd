...
## **Data**
```{r child = 'rscripts/00_preamble.Rmd', cache=TRUE}
```

```{r child = 'rscripts/01_DnY.Rmd', echo = FALSE, cache=TRUE}

```

```{r robustplot, echo=FALSE, fig.height=8, fig.width=8, cache=FALSE}
df_date <- as.Date(rownames(rw), format = '%Y-%m-%d')
ssplot_xts <- function(tab, name, df_date, w, r, c, ...){
  TC <- list()
  for(i in 1:length(tab)){
    val <- unlist(strsplit(names(tab)[i], "_"))
    TC[[i]] <- tab_roll_data(tab[[i]],r,c,window=as.numeric(val[3]))
  }
  
  TC <- do.call(cbind, TC)
  colnames(TC) <- paste0(name, names(tab)); colnames(TC)
  ...
  tc <- retainvar(TC, as.character(w))
  nn <- ncol(tc)
  ipak('xts')
  tc <- xts::xts(tc, order.by=df_date, format = '%Y-%m-%d')
  zoo::plot.zoo(tc, screens = 1, xlab="", ylab = '', col= rainbow(nn), lwd = 2,...)
  zoo::autoplot.zoo(tc)
  grid()
}
cname <- gsub("[.]", " ", colnames(rw))
```
