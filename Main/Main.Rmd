...
```{r, echo=FALSE}
descriptives <- function(y){
  ipak('tseries')
  stdy <- apply(y, 2, sd)
  z <- t( apply((y-colMeans(y)), 1, '/',stdy))
  
 val <-  cbind('Mean' = colMeans(y), #'p-val of t-test' = apply(y, 2, function(x) t.test(x)$p.value),
              'Median' = apply(y, 2, median), 'Max' =apply(y, 2, max), 'Min'= apply(y, 2, min),
              'Std dev' = apply(y, 2, sd), 'Skew' = colMeans(z^3), 'Kurt' = colMeans(z^4),
              'JB-test' = apply(y, 2, function(x) jarque.bera.test(x)$statistic),
              #'p-value' = apply(y, 2, function(x) jarque.bera.test(x)$p.value),
              'ADF-test' = apply(y, 2, function(x) adf.test(x)$statistic))#,
              #'p-value' = apply(y, 2, function(x) adf.test(x)$p.value),
              #'LB-Stat (1)' = apply(y, 2, function(x) Box.test(x, lag = 1)$statistic),
              #'P-value (1)' = apply(y, 2, function(x) Box.test(x, lag = 1)$p.value),
              #'LB-Stat (3)' = apply(y, 2, function(x) Box.test(x, lag = 3)$statistic),
              #'P-value (3)' = apply(y, 2, function(x) Box.test(x, lag = 3)$p.value),
              #'LB-Stat (5)' = apply(y, 2, function(x) Box.test(x, lag = 5)$statistic),
              #'P-value (5)' = apply(y, 2, function(x) Box.test(x, lag = 5)$p.value))
 return(val)
}

des <- round(descriptives(rw),2)
rownames(des) <- gsub(pattern = "[.]", "", rownames(des))
rownames(des) <- gsub(pattern = "1", "", rownames(des))
colnames(des) <- gsub(pattern = "[.]", "", colnames(des))
ipak('kableExtra')
kable(des, "latex", booktabs = T)
```


## **Table-2: Full Sample Table for weekly return (W = 250, h = 10)**
```{r,echo=FALSE}
## Table-2: Full Sample Table for weekly return (W = 250, h = 10)
tab_full_r <- mytable(dny(df = vardat, max_lag = 4, h = 10, window = nrow(vardat)), n_col=ncol(vardat), rname = colnames(vardat))
rownames(tab_full_r) <- gsub(pattern = "[.]", "", rownames(tab_full_r))
colnames(tab_full_r) <- gsub(pattern = "[.]", "", colnames(tab_full_r))
kable(round(tab_full_r, 2), "latex", booktabs = T)

```

## **Table-3: Full Sample Table for weekly return (W = 250, h = 10)**
```{r,echo=FALSE}
## Table-3: Full Sample Table for weekly return (W = 250, h = 10)
tab_full_v <- mytable(dny(df = wv, max_lag = 4, h = 10, window = nrow(vardat)), n_col=ncol(vardat), rname = colnames(vardat))
rownames(tab_full_v) <- gsub(pattern = "[.]", "", rownames(tab_full_v))
colnames(tab_full_v) <- gsub(pattern = "[.]", "", colnames(tab_full_v))
kable(round(tab_full_v, 2), "latex", booktabs = T)
```


## **Figure-4: 250 Weeks fixed window rolling sample for H=10**
```{r,echo=FALSE, fig.width=8, fig.height=8/1.6}
par(mfrow = c(1, 1), mai = c(0.5, 0.5, 0.25, 0.25))
## Figure 4: 250 Weeks fixed window rolling sample for H=10
TCR <- list()
for(i in 1:length(tab_rw)){
  val <- unlist(strsplit(names(tab_rw)[i], "_"))
  TCR[[i]] <- tab_roll_data(tab_rw[[i]],8,7,window=as.numeric(val[3]))
}
TCR <- do.call(cbind, TCR)
colnames(TCR) <- paste0('TCR_', names(tab_rw))
TCR <- retainvar(TCR, 'dy_10_250')

TCV <- list()
for(i in 1:length(tab_wv)){
  val <- unlist(strsplit(names(tab_wv)[i], "_"))
  TCV[[i]] <- tab_roll_data(tab_wv[[i]],8,7,window=as.numeric(val[3]))
}
TCV <- do.call(cbind, TCV)
colnames(TCV) <- paste0('TCV_', names(tab_wv))
TCV <- retainvar(TCV, 'dy_10_250')

TC <- cbind(TCR, TCV)
nn <- ncol(TC)
#ipak('xts')
TC <- xts::xts(TC, order.by=df_date, format = '%Y-%m-%d')
zoo::plot.zoo(TC, screens = 1, xlab="", ylab = '', col= c('black', 'red'), lwd = 2)
legend("topleft", inset=c(0,0), y.intersp = 1, 
       legend = c('Total connectedness (weekly returns)', 'Total connectedness (weekly volatility)'),  
       lty = 1, lwd = 2, bty = "n", col= c('black', 'red'), cex = 0.8)
grid()

```

## **Figure-5: 250 Weeks fixed window rolling sample for H=10, Net Transmission**
```{r,echo=FALSE}
## Figure 5: 250 Weeks fixed window rolling sample for H=10, Net Transmission
par(mfrow = c(2, 3), mai = c(0.25, 0.27, 0.25, 0.25))
for(k in 1:6){
  ...
  for(i in 1:length(tab_rw)){
    val <- unlist(strsplit(names(tab_rw)[i], "_"))
    TCR[[i]] <- tab_roll_data(tab_rw[[i]],15,k,window=as.numeric(val[3]))
  }
  TCR <- do.call(cbind, TCR)
  colnames(TCR) <- paste0('TCR_', names(tab_rw))
  TCR <- retainvar(TCR, 'dy_10_250')
  ...
  for(i in 1:length(tab_wv)){
    val <- unlist(strsplit(names(tab_wv)[i], "_"))
    TCV[[i]] <- tab_roll_data(tab_wv[[i]],15,k,window=as.numeric(val[3]))
  }
  TCV <- do.call(cbind, TCV)
  colnames(TCV) <- paste0('TCV_', names(tab_wv))
  TCV <- retainvar(TCV, 'dy_10_250')
  ...
  ipak('xts')
  TC <- xts::xts(TC, order.by=df_date, format = '%Y-%m-%d')
  zoo::plot.zoo(TC, screens = 1, xlab="", ylab = '', col= c('black', 'red'), 
                lwd = 2, main = paste0("A", k, ". ", colnames(vardat)[k], sep = ' '))
  abline(h = 0, col = 'blue')
  grid()
  legend("topleft", inset=c(0,0), y.intersp = 1, 
         legend = c('Returns', 'Volatility'),  
         lty = 1, lwd = 2, bty = "n", col= c('black', 'red'), cex = 0.8)
}
```


## **Figure-6: Robustness check for H and rolling window size for weekly returns**
```{r,echo=FALSE}
# Robustness Check
par(mfrow = c(2, 2), mai = c(0.5, 0.27, 0.25, 0.25))
for(p in 1:(length(slidingwindow)-1)){
  ssplot_xts(tab_rw, name = "TC_", df_date = df_date, 
             w = slidingwindow[p], r = 8, c = 7, legend.names=NULL, 
             main = paste0('A',p,'. ', slidingwindow[p], ' period window'))
}
ssplot_xts(tab_rw, name = "TC_", df_date = df_date, 
           w = slidingwindow[length(slidingwindow)], r = 8, c = 7, legend.names=NULL, 
           main = paste0('A', length(slidingwindow),'. ', slidingwindow[length(slidingwindow)], ' period window'))
legend("topleft", inset=c(0,0), y.intersp = 1, legend = c('H=1', 'H=5', 'H=10', 'H=100'),  lty = 1, bty = "n", col = rainbow(4), cex = 0.8)
```


## **Figure-7: Robustness check for H and rolling window  for weekly volatility**
```{r, echo=FALSE}
par(mfrow = c(2, 2), mai = c(0.5, 0.27, 0.25, 0.25))
for(p in 1:(length(slidingwindow)-1)){
  ssplot_xts(tab_wv, name = "TC_", df_date = df_date, 
             w = slidingwindow[p], r = 8, c = 7, legend.names=NULL, 
             main = paste0('A',p,'. ', slidingwindow[p], ' period window'))
}
ssplot_xts(tab_wv, name = "TC_", df_date = df_date, 
           w = slidingwindow[length(slidingwindow)], r = 8, c = 7, legend.names=NULL, 
           main = paste0('B', length(slidingwindow),'. ', slidingwindow[length(slidingwindow)], ' period window'))
legend("topleft", inset=c(0,0), y.intersp = 1, legend = c('H=1', 'H=5', 'H=10', 'H=100'),  lty = 1, bty = "n", col = rainbow(4), cex = 0.8)
```
